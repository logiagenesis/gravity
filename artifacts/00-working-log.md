...
| 35  | **Own measurement** | The brief's "decide the adaptive integrator by measurement" found that fixed-step integration does not drift at the shipped timesteps, it FAILS: relative energy errors of 1.2e+1 (three-body chaos, Verlet) and 1.1e+1 (close flyby) the first time two bodies pass close                                          | Adaptive power-of-two sub-stepping inside a fixed outer step. PEFRL goes from 3.1e+0 to 1.0e-6 at 3.6x the work. Twelve tests pin the invariants it was chosen to preserve                                                  |
| 36  | **Own measurement** | Velocity Verlet cannot reach that accuracy at ANY affordable cost — still 5.8e-3 at 248x the work — because its second-order truncation error is the limit, not the step size                                                                                                                                       | Recorded as the headline finding of `artifacts/12`, and the reason the integrator guidance and the planned warnings matter. Not hidden behind "adaptive fixes it"                                                           |
