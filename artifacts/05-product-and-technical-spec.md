...
**Core JTBD:** *"Show me what this gravitational system does, let me change one
thing, and let me see whether the answer can be trusted."*
...
- **Massless test particles** (flag bit 1) are excluded as *sources* but
  integrated as *targets*. Fixes the half-measure at `03` §4.6 and keeps them
...
and momentum conserved *exactly* (to floating-point tolerance) and kinetic
...
type Snapshot = {
  ...
  simTime: number; stepCount: number; bodyCount: number;
  energy: number; energyDrift: number;
  angularMomentum: number; angularMomentumDrift: number;
  stepsLastBatch: number; workerStepMs: number;
};
```
...
user. The *information* can — body list, elements, diagnostics, play state are
